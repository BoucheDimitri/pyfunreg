...
class FeaturesKPLDictsel: 
    ...
    def prox(self, alpha, stepsize):
        norms = torch.norm(alpha, p=2, dim=1)
        thresh = torch.maximum(1 - (stepsize * self.regu_dictsel) / norms, torch.tensor(0)).unsqueeze(1)
        val = alpha * thresh
        return val
    
    def obj(self, alpha):
        val = (1 / self.n) * (((self.phi @ (alpha @ self.Z.T)).T - self.Y) ** 2).sum() + self.regu * torch.diag(alpha.T @ self.phi_adj_phi @ alpha).sum()
        if torch.isinf(val):
            raise ValueError("Objective is infinite")
        return val
...
class FeaturesKPLWorking: 
    ...
    def grad(self, alpha, working=True):
        ...
        val = (2 / self.n) * (sub_gram @ alpha @ self.ZTZ - sub_Yproj @ self.Z) + 2 * self.regu * sub_gram @ alpha
        if torch.isinf((val ** 2).sum()):
            raise ValueError("Gradient with infinite norm")
        return val
    ...
    def obj(self, alpha):
        ...
        val = (1 / self.n) * (((sub_phi @ (alpha @ self.Z.T)).T - self.Y) ** 2).sum() + self.regu * torch.diag(alpha.T @ sub_gram @ alpha).sum()
        if torch.isinf(val):
            raise ValueError("Objective is infinite")
        return val
