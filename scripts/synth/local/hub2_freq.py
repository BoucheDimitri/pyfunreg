...
from datasets import add_gp_outliers, load_gp_dataset, add_local_outliers
...
if __name__ == "__main__":
    ...
    loss_params = np.flip(np.geomspace(10 ** (-2.3), 10 ** (-3), 10))
    ...
    accproxgd = AccProxGD(n_epoch=20000, stepsize0=1,
                          tol=1e-6, acc_temper=20, verbose=False)
    ...
    for i in range(n_averaging):
        ...
        conf = {"regu": lbda_grid, "loss": None, "features": nysfeat, "phi": phi,
                "refit_features": True, "center_out": False, "optimizer": accproxgd}
        ...
        with open(out_folder + "loc_hub2_freq_light" + str(i) + ".pkl", "wb") as outp:
