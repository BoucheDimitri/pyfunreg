...
def cv_consecutive(esti, losses, X, Y, K=None, Yeval=None, n_splits=5, reduce_stat="median", random_state=342):
    ...
    if isinstance(X, torch.Tensor):
        mses = torch.zeros((n_splits, len(losses)))
    else:
        mses = np.zeros((n_splits, len(losses)))
    count = 0
    for train_index, test_index in kf.split(X):
        ...
        for l, loss in enumerate(losses):
            ...
            if Yeval is not None:
                mses[count, l] = compute_mse(
                    preds, [Yeval[j] for j in test_index])
            else:
...
def cv_features(esti, features, X, Y, Ks=None, Yeval=None, phis=None, n_splits=5, reduce_stat="median", random_state=342):
    kf = KFold(n_splits=n_splits, shuffle=True, random_state=random_state)
    if isinstance(X, torch.Tensor):
        mses = torch.zeros((n_splits, len(features)))
    else:
        mses = np.zeros((n_splits, len(features)))
    count = 0
    for train_index, test_index in kf.split(X):
        Xtrain, Xtest = X[train_index], X[test_index]
        Ytrain, Ytest = Y[train_index], Y[test_index]
...
def cv(esti, X, Y, K=None, Yeval=None, n_splits=5, reduce_stat="median", random_state=342):
    kf = KFold(n_splits=n_splits, shuffle=True, random_state=random_state)
    if isinstance(X, torch.Tensor):
        mses = torch.zeros(n_splits)
    else:
    ...
    for train_index, test_index in kf.split(X):
        ...
        preds = esti.predict(Xtest, Ktest)
        if Yeval is not None:
            mses[count] = compute_mse(preds, [Yeval[j] for j in test_index])
        else:
            mses[count] = compute_mse(preds, Ytest)
        # print(count)
        print(count)
        count += 1
    if reduce_stat == "mean":
        ...
        if isinstance(X, torch.Tensor):
            return torch.tensor(mses).quantile(0.5)
        else:
...
def tune_consecutive(estis, losses, X, Y, K=None,
    ...
    with parallel_backend("loky"):
        mses = Parallel(n_jobs=n_jobs)(
            delayed(cv_consecutive)(esti, losses, X, Y, K,
                                    Yeval, n_splits, reduce_stat, random_state)
            for esti in estis)
    ...
    esti_argmin, losses_argmin = mses.argmin(
    ) // len(losses), mses.argmin() % len(losses)
...
def tune_features(estis, features, X, Y, Ks=None,
    ...
    #         for esti in estis)
    mses = [cv_features(esti, features, X, Y, Ks, Yeval, phis,
                        n_splits, reduce_stat, random_state) for esti in estis]
    mses = torch.stack(mses)
    esti_argmin, feats_argmin = mses.argmin(
    ) // len(features), mses.argmin() % len(features)
    best_esti = estis[esti_argmin]
...
def tune(estis, X, Y, K=None,
         Yeval=None, n_splits=5, reduce_stat="median",
         random_state=342, n_jobs=-1):
    # with parallel_backend("loky"):
    #     mses = Parallel(n_jobs=n_jobs)(
    #         delayed(cv)(esti, X, Y, K, Yeval, n_splits,
    #                     reduce_stat, random_state)
    #         for esti in estis)
    mses = [cv(esti, X, Y, K, Yeval, n_splits, reduce_stat, random_state) for esti in estis]
    mses = np.array(mses)
